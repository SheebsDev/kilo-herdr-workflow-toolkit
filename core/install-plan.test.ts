...
test("all selection plans each harness exactly once across Kilo-only and all-harness scenarios", async () => {
  const kiloFixture = await createCheckoutFixture();
  try {
    const plan = await buildInstallPlan({
      scope: "project",
      checkoutRoot: kiloFixture.checkout,
      destinationRoot: kiloFixture.destination,
    ...
    await rm(kiloFixture.root, { recursive: true, force: true });
  }
...
test("invalid dependencies, malformed configuration, and trust failures prevent planning", async (t) => {
  await t.test("missing default dependencies", async () => {
    const fixture = await createCheckoutFixture();
    try {
      await assert.rejects(
        ...
        (error: unknown) => error instanceof InstallPreflightError &&
          /checkout dependencies preflight failed/i.test(error.message),
      );
    } finally {
      await rm(fixture.root, { recursive: true, force: true });
    }
  ...
  await t.test("malformed TOML", async () => {
    const fixture = await createCheckoutFixture();
    await writeFile(path.join(fixture.destination, "codex.toml"), "mcp_servers = [\n");
    try {
      await assert.rejects(
        buildInstallPlan({
          scope: "project",
        ...
        (error: unknown) => error instanceof InstallPreflightError &&
          /configuration.*not valid TOML/i.test(error.message),
      );
    } finally {
      await rm(fixture.root, { recursive: true, force: true });
    }
  ...
  await t.test("malformed JSON", async () => {
    ...
    try {
      await assert.rejects(
        ...
        (error: unknown) => error instanceof InstallPreflightError &&
          /configuration.*not valid JSON/i.test(error.message),
      );
    } finally {
      await rm(fixture.root, { recursive: true, force: true });
    }
  ...
  await t.test("trust failure", async () => {
    const fixture = await createCheckoutFixture();
    try {
      await assert.rejects(
        buildInstallPlan({
        ...
        (error: unknown) => error instanceof InstallPreflightError &&
          /Project trust is required/i.test(error.message),
      );
    } finally {
      await rm(fixture.root, { recursive: true, force: true });
    }
...
test("uninstall removes unchanged file and dependency ownership while retaining modified or shared content", async () => {
  const fixture = await createCheckoutFixture();
